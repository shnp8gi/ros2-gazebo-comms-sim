with open('src/comms_sim_pkg/src/comms_node.cpp', 'w') as f:
    f.write(r"""#include "comms_sim_pkg/comms_node.hpp"
  ...
  last_rssi_(0.0)
{
  ...
  propagation_model_ = std::make_unique<LogDistancePathLossModel>(c, freq, exp, d0, pl_d0);

  comms_calculator_ = std::make_unique<CommsCalculator>(
    propagation_model_.get(), tx_power_, noise_variance_, mcs_table_path_);

  rssi_threshold_ = comms_calculator_->get_rssi_min();

  double max_att = this->get_parameter("max_antenna_attenuation").as_double();
  antenna_parser_.set_max_attenuation(max_att);
...
void CommsSimulatorNode::on_base_station_pose(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
  // Ignored for array based base stations
}
...
void CommsSimulatorNode::calculate_and_publish() {
    ...
    for (size_t i = 0; i < base_stations_.size(); ++i) {
        ...
        double tx_e, tx_h, tx_tot, rx_e, rx_h, rx_tot;
        antenna_parser_.get_tx_rx_gains(
            bs_antenna_pos, bs_ant_rpy, ugv_antenna_pos, ugv_ant_rpy,
            bs.rotmat, rx_rotmat, tx_e, tx_h, tx_tot, rx_e, rx_h, rx_tot);
