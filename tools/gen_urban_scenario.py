        ...
        'config_overrides': {
            'link_controller_node': {
                'ros__parameters': {
                    ...
                    # 平均場の事前値。地図は「ここからのずれ」を学習する。
                    # 0 dBm のままだと未観測の位置を「極めて強い信号」と評価し、
                    # 悲観的であるべき LCB が逆に楽観的になる (実測: 道路の
                    # 35-46% が未観測、そのうち 27-41% で接続閾値超え = 道路の
                    # 約12%で幻のリンクを掴む)。排他接続なので幻の grant は
                    # 実在する車を締め出す純粋な害。熱雑音床相当に置く
