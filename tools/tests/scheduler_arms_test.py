...
def main():
    ...
    try:
        ...
        # 忘却: P が load 時に膨らむ
        # 地図キーは (BS index, 進行方向, アンテナ)。本テストの車は +x 方向・1本
        p_saved = pred.maps[(1, 1, 0)].P
        if not (np.diag(pred2.maps[(1, 1, 0)].P) > np.diag(p_saved) + 0.049).all():
        ...
        for run in range(15):
            ...
            while s < 300.0:
                ...
                t += 0.05
                s = 16.67 * t
        var_in = pred3.varmaps[(1, 1, 0)].query(110.0)
        var_out = pred3.varmaps[(1, 1, 0)].query(60.0)
