...
## P2. 交通生成 — 対象車の混在 (A3, A5) — **完了 (2026-07-27)**

### 成果物

| ファイル | 内容 |
|---|---|
| `tools/lib/traffic_gen.py` | 対象車 (role: tx) 生成・`prefill`・`interleave_arrivals` を追加 |
| `tools/lib/scenario_loader.py` | 遮蔽ジオメトリの登録を role から独立させ、`role: tx` も `blockers` に載せる |
| `tools/tests/traffic_targets_test.py` | 新機能 33 件、全緑 |

### 後方互換の検証

`road_10car.yaml` について **旧実装と新実装の展開結果をビット単位で比較**し、
5 シード (0/1/42/12345/999999) すべてでエンティティ列が完全一致することを確認した。

これを保証するための設計: **`target_ratio` が 0 のときは is_target 判定の乱数を引かない**。
乱数列の消費順が従来と一致するため、過去の交通実現が再現される。同様に `prefill` 省略時は
到着列の開始時刻が従来どおり `t0 + headway` になる。

### 実測した交通量 (仕様 §2.3 の表に反映)

`over` 条件で **t=0 のコリドー内対象車が 12.0 台 vs RSU 4 台** = 3 倍の需要超過。
当初の解析見積り (5.8 台) より強い。先着占有の独占が明確に出る水準。

同時に **`over` では対象車が 40 台生成される**ことが判明し、リンク評価が
160/tick (road_10car の 5.3 倍) になるため **コリドーゲーティング (P4) が必須**と確定した。
